...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for r in range(rows) :
            for c in range(cols) :
                ...
                std = Const.SONAR_STD
                mean = math.sqrt((agentY-util.rowToY(r))**2+(agentX-util.colToX(c))**2)
                value = observedDist
                prob = prob * util.pdf(mean, std, value)
                self.belief.setProb(r, c, prob)
    ...
    def elapseTime(self) -> None:
        ...
        rows = self.belief.getNumRows()
        cols = self.belief.getNumCols()
        cur = [[self.belief.getProb(r,c) for c in range(cols)] for r in range(rows)]
        for r in range(rows) :
            for c in range(cols) :
                prob = 0
                for r1 in range(rows) :
                    for c1 in range(cols) :
                        if ((r1,c1),(r,c)) in self.transProb :
                            prob += cur[r1][c1] * self.transProb[((r1,c1),(r,c))]
                self.belief.setProb(r, c, prob)
        self.belief.normalize()
        #raise Exception("Not implemented yet")
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for p in self.particles :
            prob = self.particles.get((p[0],p[1]))/200
            mean = math.sqrt((agentY-util.rowToY(p[0]))**2+(agentX-util.colToX(p[1]))**2)
            prob = prob * util.pdf(mean, Const.SONAR_STD, observedDist)
            self.particles[p] = prob 
        new_particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES) :
            new_particles[util.weightedRandomChoice(self.particles)] += 1
    ...
    def elapseTime(self) -> None:
        ...
        for newtile in self.particles: 
            for _ in range(self.particles[newtile]) :
                new[util.weightedRandomChoice(self.transProbDict[newtile])] += 1
        self.particles = new
